...
'''Tests for fvirt.commands.domain.undefine'''
...
def test_command_run(
    ...
    result = runner(('-c', uri, 'domain', 'undefine', dom.name), 0)
    assert len(result.output) > 0

    assert dom not in hv.domains
...
def test_command_bulk_run(
    ...
    assert all(d.valid for d in doms)
    ...
    result = runner(('-c', uri, 'domain', 'undefine', '--match', 'name', object_name_prefix), 0)
    assert len(result.output) > 0

    assert all(d not in hv.domains for d in doms)
