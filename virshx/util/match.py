...
'''libvirt object matching tooling for virshx CLI.'''
...
MATCH_HELP = '''virshx object matching is based on two parameters passed to the --match
option of a virshx command. The first is the match target, and the second
...
the specific property of the ojbect that will be matched against.
...
class MatchPatternParam(click.ParamType):
    '''Class for processing match patterns.
       ...
       object for use with the virshx.util.match.matcher() function.'''
    name = 'pattern'
